...
class NewPressureViewModel(
    private val pressureDao: PressureDao,
    application: Application
) : AndroidViewModel(application) {
    private var insertedPressureId = 0L
    ...
    init {
        _measurementDate.value = LocalDate.now()
        _measurementTime.value = LocalTime.now().withNano(0).withSecond(0)
    }
    ...
    private suspend fun insertPressure() {
        ...
        withContext(Dispatchers.IO) {
            insertedPressureId = pressureDao.insert(pressure)
        }
    }
